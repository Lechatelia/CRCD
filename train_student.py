...
def parse_option():
    ...
    # kd with gard
    parser.add_argument('--with_grad', type=bool, default=True, help='whether to use gard for kd')
    parser.add_argument('--grad_kd_type', type=str, default='mse', choices=['mse', 'crd', 'crcd'])
    parser.add_argument('--grad_kd_weight', type=float, default=1.0)
    # KL distillation
    ...
    parser.add_argument('--embed_type', default='linear', type=str, choices=['nofc', 'linear', 'nonlinear'])
    ...
    opt.model_name = 'S{}_T:{}_{}_{}_r{}_a:{}_b:{}_{}_k:{}_T:{}_{}'.format(opt.model_s, opt.model_t, opt.dataset, opt.distill,
                                                                opt.gamma, opt.alpha, opt.beta, opt.trial, opt.nce_k, opt.nce_t, time.time())
...
def main():
    ...
    if opt.path_t is not None:
        print("load teacher model from {}".format(opt.path_t))
        model_t = load_teacher(opt.path_t, n_cls)

    else:
        print(" there is no teacher available, Resnet50 without pretrained will works")
        model_t = model_dict['ResNet50'](num_classes=n_cls)
    print("student model {} training --->".format(opt.model_s))
    ...
    if opt.with_grad:
        if opt.grad_kd_type == "crd":
            opt.s_dim = feat_s[-1].shape[1]
            opt.t_dim = feat_t[-1].shape[1]
            opt.n_data = n_data
            criterion_kd_grad = CRDLoss(opt)
            module_list.append(criterion_kd_grad.embed_s)
            module_list.append(criterion_kd_grad.embed_t)
            trainable_list.append(criterion_kd_grad.embed_s)
            trainable_list.append(criterion_kd_grad.embed_t)
        elif opt.grad_kd_type == 'crcd':
            opt.s_dim = feat_s[-1].shape[1]
            opt.t_dim = feat_t[-1].shape[1]
            opt.n_data = n_data
            criterion_kd_grad = CRCDLoss(opt)
            module_list.append(criterion_kd_grad.embed_s)
            module_list.append(criterion_kd_grad.embed_t)
            trainable_list.append(criterion_kd_grad.embed_s)
            trainable_list.append(criterion_kd_grad.embed_t)
